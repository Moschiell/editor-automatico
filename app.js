...
$('generate').onclick=async()=>{
  ...
  try{
    ...
    selected.forEach(file=>f.append('videos',file,file.name));
    const wm=$('watermark').files[0];
    if(wm)f.append('watermark',wm,wm.name);
    ['template','headline','caption','handle','logoSize','speed'].forEach(id=>f.append(id,$(id).value));
    f.append('mirror',$('mirror').checked?'true':'false');
    setStatus(wm?"Enviando vídeo + marca d'água...":'Enviando vídeos...',0);
    const r=await fetch(API+'/api/jobs',{method:'POST',body:f,credentials:'omit'});
