(() => {
  ...
  async function toBlobURL(url, mime) {
    ...
    if (!r.ok) throw new Error(`Não foi possível carregar ${url} (${r.status}).`);
  ...
  function capturePoster() {
    ...
    try {
      ...
      posterURL = canvas.toBlob ? null : posterURL;
      canvas.toBlob(blob => {
        if (!blob) return;
        if (posterURL) URL.revokeObjectURL(posterURL);
      ...
      console.warn("Não foi possível criar o fundo do preview", e);
    }
  }
  ...
  // File selection is intentionally independent from FFmpeg loading.
  ...
  async function processOne(file, idx, engine) {
    ...
    let logoName = null;
    let fontName = "DejaVuSans-Bold.ttf";
    ...
    try {
      ...
      // Background uses the original stream. Foreground uses the same video and
      // is scaled to fit inside the 1080x1920 canvas.
      ...
      if (logoName) try { await engine.deleteFile(logoName); } catch {}
    }
  }
