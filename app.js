(() => {
  ...
  async function loadFFmpeg() {
    ...
    try {
      ...
      // Worker is same-origin; only core/WASM come from CDN.
      const classWorkerURL = new URL("./ffmpeg-worker.js?v=20260916-04", location.href).href;
      await ffmpeg.load({ corePath: CORE_URL, classWorkerURL, coreURL: CORE_URL, wasmURL: WASM_URL });

      const fontResponse = await fetch("DejaVuSans-Bold.ttf?v=20260916-03", { cache: "force-cache" });
      if (!fontResponse.ok) throw new Error("Não foi possível carregar a fonte do texto.");
      ffmpeg.FS("writeFile", "DejaVuSans-Bold.ttf", new Uint8Array(await fontResponse.arrayBuffer()));
      fontReady = true;
  ...
  function fsDelete(name) { try { ffmpeg.FS("unlink", name); } catch {} }
  ...
  async function processOne(file, idx, engine) {
    ...
    try {
      ...
      engine.FS("writeFile", input, new Uint8Array(await file.arrayBuffer()));

      if (logoFile) {
        const logoExt = (logoFile.name.split(".").pop() || "png").toLowerCase().replace(/[^a-z0-9]/g, "") || "png";
        logoName = `logo_${id}.${logoExt}`;
        engine.FS("writeFile", logoName, new Uint8Array(await logoFile.arrayBuffer()));
      }
  ...
  $("process").addEventListener("click", async () => {
    ...
    try {
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        link.href = url; link.download = name; link.textContent = "Baixar vídeo";
        const video = document.createElement("video");
        video.controls = true; video.playsInline = true; video.src = url;
        div.append(title, br, link, video);
